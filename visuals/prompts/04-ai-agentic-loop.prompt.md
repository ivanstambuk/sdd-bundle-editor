...
"Spec Studio — The Agentic Reasoning Loop"

Goal:
- Draw a single, visually striking infographic showing how an external AI Agent (like GitHub Copilot or Claude Desktop) leverages Spec Studio's MCP tools to autonomously navigate the ontology and fulfill complex tasks like code generation or reporting.
- The narrative arc is left→right: User Intent → Agentic Loop (interacting with MCP) → Fulfilling Action.

Overall layout:
- Partition the canvas into three regions horizontally (left, center, right).
- Place a title at the top center: "The Agentic Reasoning Loop via MCP" in a large, bold heading.

Left region — "The User Intent":
- Region heading: "User Request".
- Draw a chat box or user prompt icon with two divergent examples representing different intents:
  1. A short label reading: "Query: 'What is the impact of changing Domain-X?'"
  2. Another label reading: "Command: 'Implement scaffolding for REQ-LOGIN-01'"
- A clear arrow points from these prompts into the center region.

Center region — "The Agentic Loop & MCP Toolkit":
- Region heading: "AI Agent Reasoning Loop".
- Draw a central circular or spiral flow (an iteration loop) representing the agent's thought process (e.g., Plan → Navigate → Reason).
- Adjacent to the loop, draw a utility box or "Toolbelt" representing the Spec Studio MCP Tools. 
- Show bidirectional arrows connecting the Agent Loop to the Toolbelt.
- Add small icons and labels for the specific MCP tools being invoked during the loop: 
  - `search_entities`
  - `read_entities`
  - `get_context` 
  - `apply_changes`
...
- Region heading: "Fulfilling Action".
- Show two branch outputs mapping to the original user intent:
  1. A neat report or document icon. Label: "Accurate Architectural Query Answer"
  2. A code block or IDE icon. Label: "Generated Implementations & Code Scaffolding"
- Ensure confident, solid arrows pointing from the Agentic Loop to these final outcomes.

Visual connections:
- A clear left→right timeline flow, highlighting that the AI loop in the middle is powered exclusively by its interactions with the MCP Toolbelt.

Footer sentence (bottom strip):
Agents recursively search, read, and interpret Spec Studio's ontology via MCP tools to autonomously fulfill complex generation and querying tasks.
